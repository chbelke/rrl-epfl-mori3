...
classdef RigidMori < robotics.manip.internal.InternalAccess
...
%   as bodyname.
%
%   body = rigidBody(bodyname, 'MaxNumCollisions', N) allows the user to specify an
%   upper bound on the number of collision geometries allowed in the body
...
%#codegen
    ...
    methods
        function obj = RigidMori(bname, varargin )
        %rigidBody Constructor

            bname = convertStringsToChars(bname);

            if coder.target('MATLAB')
                if nargin == 5 %length(varargin) = 4 + bname
                    if (matches(varargin{1}, 'BodyInternal') && matches(varargin{3}, 'TreeInternal'))
                        bodyInternal = varargin{2};
                        treeInternal = varargin{4};
                    end
                else
                    parser = inputParser;
                    parser.addParameter('BodyInternal', RigidMoriInternal(bname) ); %%NOTE !!!Modified!!!
...
%FIXME sketchy but speeds up code significantly
%                     validateattributes(bodyInternal,
%                     {'robotics.manip.internal.RigidBody'}, ...
...
%                                        {'scalar', 'nonempty'},'rigidBody', 'the third input argument');
                ...
                bodyInternal = coder.internal.getParameterValue(pstruct.BodyInternal, ...
                                                                RigidMoriInternal(bname), varargin{:}); %%NOTE !!!Modified for coder!!!
        ...
        %   mesh file must be in STL format.
        %
        ...
        %       addVisual(b1, 'Mesh', 'meshes/B1.stl', T);
    ...
    methods
        ...
        %get.Parent
            parent = RigidMori('-', 'BodyInternal', obj.TreeInternal.parentOf(obj.Name),... %NOTE !!!MODIFIED!!!
        ...
        %get.Children
            ...
            for i = coder.unroll(1:obj.TreeInternal.MaxNumBodies)
                if i <= l
                    children{i} = RigidMori('-', 'BodyInternal', childrenInternal{i}, ... %%NOTE !!!Modified!!!
                                            'TreeInternal', obj.TreeInternal);
                end
            end
        end
...
%% User code
    ...
    methods
        ...
        %get.Child
            childrenInternal = obj.TreeInternal.childrenOf(obj.Name);
            assert(i<=length(childrenInternal), 'Child index out of bounds');
                    child = RigidMori('-', 'BodyInternal', childrenInternal{i}, ...
                                            'TreeInternal', obj.TreeInternal);
        end

        function value = get.ChildLocation(obj)
            %get.ChildLocation
            value = obj.BodyInternal.ChildLocation;
        end
        
        function set.ChildLocation(obj, value)
            %set.ChildLocation
            obj.BodyInternal.ChildLocation = value;
        end
        
        function set.EdgeLengths(obj, value)
            %set.EdgeLengths
            obj.BodyInternal.EdgeLengths = value;
        end
        
        function value = get.EdgeLengths(obj)
            %get.EdgeLengths
            value = obj.BodyInternal.EdgeLengths;
        end
        
        function value = get.xyt1t2(obj)
            %get.xyt1t2
            value = obj.BodyInternal.xyt1t2;
        end
        
        function set.xyt1t2(obj, value)
            %set.xyt1t2
            obj.BodyInternal.xyt1t2 = value;
        end
        
        function value = get.updateColMesh(obj)
            %get.updateColMesh
            value = obj.BodyInternal.updateColMesh;
        end
        
        function set.updateColMesh(obj, value)
            %set.updateColMesh
            obj.BodyInternal.updateColMesh = value;
        end
    end
end
