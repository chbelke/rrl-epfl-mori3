...
volatile short ACC_Data[3] = {0, 0, 0};
...
void MMA8452Q_Read(void) {
    ...
    uint8_t i;
    for (i = 0; i < 5; i += 2){
        ACC_Data[i/2] = ((uint16_t)(readBuffer[i] << 8 | readBuffer[i+1])) >> 4;
    }
}

uint16_t ACC_Get(uint8_t axis) { //axis 0 corresponds to x, 1 to y, 2 to z
