...
void callback(char* topic, byte* payload, unsigned int len)
{
  ...
  else if (!memcmp(payload2,"hello",sizeof("hello")-1)) //'he' is for hello
  {
    client.publish(publishName, "INFO: Hello!");
  }
  
  else if (!memcmp(payload2,"hello",sizeof("hello")-1)) //'com' is for communication
  {  
    char text[32];
  ...
  else if (!memcmp(payload2,"hand",sizeof("hand")-1)){ //'ha' is for handshake
